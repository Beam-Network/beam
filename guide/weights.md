...
```text
base_raw_i = verified_uploaded_bytes_i * penalty_multiplier_i
```
...
| Input             | Source                                                   |
| ----------------- | -------------------------------------------------------- |
| `verified_uploaded_bytes` | Sum of server-planned chunk bytes from completed production tasks in the PRISM evidence window (7 days by default) |
...
- Complete enough penalty-adjusted uploaded production bytes inside the PRISM evidence window to rank into a higher emission tier.
