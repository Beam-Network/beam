...
Run with: python -m actors.orchestrator.main

The Orchestrator coordinates bandwidth tasks with BeamCore:
- Registers with BeamCore HTTP on startup
- Keeps a live NATS control session to the orchestrator gateway (upstream BeamCore relay) for assignments and control updates
...
| ORCHESTRATOR                                                       |
|                                                                    |
| BeamCore <---- Register / NATS control ----> Assignments           |
|    |                                           |                    |
|    v                                           v                    |
...
async def lifespan(app: FastAPI):
    ...
    if orchestrator.subnet_core_client:
        ...
        if api_key:
            ...
            logger.info(
                "SubnetCoreClient API key: not fetched yet - first NATS control connection connect "
...
- Maintains a live NATS control connection control-plane session
