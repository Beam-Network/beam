...
class OrchestratorSettings(BaseSettings):
    ...
    api_host: str = Field(
        default="0.0.0.0",
        validation_alias=AliasChoices("ORCHESTRATOR_HOST", "API_HOST"),
    ...
    uid: Optional[int] = Field(
        default=None,
        validation_alias=AliasChoices("ORCHESTRATOR_UID", "UID"),
        env="ORCHESTRATOR_UID",
    )
    ...
    worker_gateway_url: Optional[str] = Field(
        ...
        env="ORCHESTRATOR_WORKER_GATEWAY_URL",
    )
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.orch_gateway_url.startswith(("http://", "https://", "ws://", "wss://")):
            raise ValueError("ORCH_GATEWAY_URL must be a NATS endpoint, not HTTP/WebSocket")
