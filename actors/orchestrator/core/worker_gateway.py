...
TRANSFER_MULTIPART_CAPABILITY = "transfer.multipart"
CAPABILITY_TTL_SECONDS = 45


def _rfc3339_now(offset_seconds: float = 0.0) -> str:
    return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time() + offset_seconds))


def _parse_rfc3339(value: object) -> float:
...
def _supported_manifest_capabilities(manifest: dict) -> set[str]:
    capacity = manifest.get("capacity")
    if not isinstance(capacity, dict):
        return set()
    try:
        if int(capacity.get("available_connections", 0)) <= 0:
            return set()
    ...
    for protocol in manifest.get("protocols", []):
        ...
        if not isinstance(name, str) or not name.strip():
            continue
        try:
            protocol_min = int(protocol.get("min", 0))
            protocol_max = int(protocol.get("max", 0))
        except (TypeError, ValueError):
            continue
        if protocol_min <= 1 <= protocol_max:
            supported_protocols.add(name.strip())

    capabilities = set()
    for capability in manifest.get("capabilities", []):
        if isinstance(capability, str) and capability.strip() in supported_protocols:
...
class WorkerGateway:
    ...
    def __init__(
        ...
        self._worker_capability_manifests: Dict[str, dict] = {}
        self._capability_update_task: Optional[asyncio.Task] = None
    ...
    def refresh_capabilities(self) -> None:
        self._schedule_capability_update()
    ...
    def disconnect(self, worker_id: str) -> None:
        ...
        self._worker_capability_manifests.pop(worker_id, None)
    ...
    async def handle_worker_message(self, worker_id: str, raw: str) -> None:
        ...
        elif msg_type == "worker_capability_update":
            self._handle_worker_capability_update(worker_id, msg)
        else:
            logger.debug("Unhandled worker message type %s from %s", msg_type, worker_id)

    def _handle_worker_capability_update(self, worker_id: str, msg: dict) -> None:
        manifest = msg.get("capability_manifest")
        if not isinstance(manifest, dict):
            logger.warning("Worker %s sent invalid capability manifest", worker_id)
            return
        if (
            manifest.get("schema_version") != CAPABILITY_SCHEMA_VERSION
            or manifest.get("actor_type") != "worker"
            or manifest.get("actor_id") != worker_id
            or not isinstance(manifest.get("software_version"), str)
            or not isinstance(manifest.get("capabilities"), list)
            or not isinstance(manifest.get("protocols"), list)
            or not isinstance(manifest.get("capacity"), dict)
        ):
            logger.warning("Worker %s sent rejected capability manifest", worker_id)
            return
        if _parse_rfc3339(manifest.get("observed_at")) <= 0:
            logger.warning("Worker %s sent capability manifest without observed_at", worker_id)
            return
        if _parse_rfc3339(manifest.get("expires_at")) <= time.time():
            logger.warning("Worker %s sent expired capability manifest", worker_id)
            return
        self._worker_capability_manifests[worker_id] = manifest
        self._schedule_capability_update()

    def _schedule_capability_update(self) -> None:
        if self._capability_update_task and not self._capability_update_task.done():
            return
        try:
            asyncio.get_running_loop()
        except RuntimeError:
            return
        self._capability_update_task = asyncio.create_task(self._publish_capability_update())

    async def _publish_capability_update(self) -> None:
        try:
            if self._upstream is None or not hasattr(self._upstream, "send_capability_update"):
                return
            manifest = self._orchestrator_capability_manifest()
            await self._upstream.send_capability_update(manifest)
        except Exception as exc:
            logger.info("capability_update publish skipped: %s", exc)
        finally:
            self._capability_update_task = None

    def _orchestrator_capability_manifest(self) -> dict:
        capabilities = set()
        now = time.time()
        for worker_id in self._sessions:
            manifest = self._worker_capability_manifests.get(worker_id)
            if manifest is None:
                capabilities.add(TRANSFER_MULTIPART_CAPABILITY)
                continue
            if _parse_rfc3339(manifest.get("expires_at")) <= now:
                continue
            capabilities.update(_supported_manifest_capabilities(manifest))
        ordered = sorted(capabilities)
        available = 1 if ordered else 0
        hotkey = str(getattr(self._upstream, "orchestrator_hotkey", "") or "")
        version = str(getattr(self._upstream, "software_version", "") or "0.2.0")
        return {
            "schema_version": CAPABILITY_SCHEMA_VERSION,
            "actor_type": "orchestrator",
            "actor_id": hotkey,
            "software_version": version,
            "protocols": [{"name": capability, "min": 1, "max": 1} for capability in ordered],
            "capabilities": ordered,
            "capacity": {"max_connections": available, "available_connections": available},
            "observed_at": _rfc3339_now(),
            "expires_at": _rfc3339_now(CAPABILITY_TTL_SECONDS),
        }
