...
| `ORCH_GATEWAY_URL`                | BeamCore NATS control endpoint                      | `tls://orch-gateway.b1m.ai:4222` |
...
Set `ORCH_GATEWAY_URL` to a NATS endpoint using `nats://` or `tls://`. `READY` defaults to `false` in code. Set it to `true` when the orchestrator should receive production task offers.
...
The orchestrator publishes a canonical `capability_update` after registration and after each heartbeat. It derives that manifest from fresh worker manifests currently connected through the worker gateway.

`transfer.multipart` is the baseline normal-transfer capability. Workers without a fresh canonical manifest are treated as legacy baseline transfer workers only.
